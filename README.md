...
My solution consists of two files, both of them can be found in `scripts` folder. `agent.py` contains my implemantation of the AI-agent that fixes buggy code. For this task I went with two different models: 'Qwen3-0.6B' from Qwen and 'claude-sonnet-4-20250514' from Anthropic. First one runs locally, the second one sends requests to the server. 
...
It's **important** to mention that this code was ran on M3 Pro processor. I've added the support of cuda, but did not test it on CUDA-device. 
...
To evaluate model's performance pass@1 metric was introduced. It shows the success rate of one-shot solution. I've ran the whole evaluation pipeline three times: for Qwen model with max_attempts=5 and for Anthropic model with max_attempts=3 and 5. JSON files in `results` folder contain mapping 'test_id — result(Boolean)'. 

The maximal number of tokens for models differ, foe Qwen it is set to 200, for Clause to 500. I did it because Qwen model tends to repeat the same sentence on the reasoning stage. Example output:
...
| claude-sonnet-4-20250514    | 5   | 1.00| 
